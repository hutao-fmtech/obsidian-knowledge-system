...
| agent 共享记忆（交接板、用户画像、经验、决策暂存） | `Agent Memory` |
| agent 会话记录（一次会话的意图、决策、结果、遗留） | `Agent Session` |
...
**继承**：子目录没有自己的 `slot` 时，沿用最近上级 README 的契约。只在收录内容发生变化的边界写契约；按主题细分的子目录（如 `技术文档/操作系统`）不需要单独写。空槽位收录的类型与上级不同时必须写契约（它没有内容可以反推用途）；与上级相同则直接继承。和相邻槽位容易混淆时（如 `20-日志/周记` 与 `自省/每周复盘`），即使 type 相同也写 README 说明边界。`README` 类型在任何槽位都合法，不必列入 `accepts`。`00-系统/` 与 `90-附件/` 不是内容槽位，不做契约检查；例外是 `00-系统/agent-memory/`，它有自己的契约，照常检查。
...
**检查**：`/vault-lint`（脚本 `.agents/skills/vault-lint/vault_lint.py`）按契约和本文件检查全库。脚本在运行时直接解析本文件的 `type` 表、`status` 列表和业务状态表：新增类型或枚举值只需改这里；改动这几张表的格式（列顺序、反引号写法）时，要同步检查脚本能否解析。
...
- **`visibility`**：三种状态，含义不同
  - **不写**：默认私有，大多数笔记都是这样
  - **`"🌐 可公开"`**：确认可以对外公开，用来筛选 `30-输出/` 的候选素材
  - **`"🔒 私有"`（显式写）**：**含敏感信息，禁止改成可公开**。敏感信息指证件号、密码或口令、手机号、密钥等，也包括他人的联系方式。它比"不写"更强：提醒任何人和 agent，这篇笔记不能出现在输出素材里
  - vault-lint 会检查：标了 🌐 却含敏感信息 → error（`PRIVACY_PUBLIC`）；含敏感信息却没显式标 🔒 → warn（`SENSITIVE_UNMARKED`）。检测规则写在脚本里，只认有固定格式的信息；财务数字、他人评价这类没有固定格式的隐私，要靠写作者自己判断
